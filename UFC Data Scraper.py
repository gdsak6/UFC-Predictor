...
Find URLs at: http://ufcstats.com/statistics/events/completed"""
    ...
    def get_event_info(self, event_url):
        try:
            ...
            return {'event_name': event_name, 'event_date': event_date, 'event_location': event_location}
        except Exception as e:
    ...
    def scrape_fight_details(self, fight_url, event_data):
        try:
            ...
            winner = 'Red' if fighter_statuses[0] == 'W' else 'Blue'
            ...
            if any(x in method for x in ['Decision - Unanimous', 'Decision - Split', 'Decision - Majority']):
                method = 'Decision'
            if any(x in method for x in ["Doctor's Stoppage", 'DQ']):
                method = 'KO/TKO'
    ...
    def create_large_dataset(self, event_urls):
        ...
        df['r_current_age'] = df['r_date_of_birth'].apply(lambda x: calc_age(x, datetime.now().date()) if pd.notna(x) else None)
        df['b_current_age'] = df['b_date_of_birth'].apply(lambda x: calc_age(x, datetime.now().date()) if pd.notna(x) else None)
        ...
        column_order = [
            'event_date', 'event_name', 'event_location', 'r_fighter', 'b_fighter',
